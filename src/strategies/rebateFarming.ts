...
export function getRebateFarmingStrategy(maxLeverage: number): StrategyParams {
  ...
  return {
    // ==================== 策略基本信息 ====================
    name: "返佣套利",  // 策略名称（中文）
    description: "超高频微利交易，只看1m和3m线，不允许空仓，专注手续费返佣累积",  // 策略描述
...
5. **空仓处理流程**：
   - 检测到空仓 → 立即扫描所有币种的1m和3m线
   - 选择1m和3m同向最明确的1-2个币种
   - 如果所有币种都不明确，选择波动最大的币种（波动=机会）
...
3. 快速试错：超短线策略允许试错，错了就快速止损
4. 持仓时间：单笔持仓超过60分钟需警惕，考虑平仓
5. 依赖止损保护：不要害怕开仓，有代码级止损保护

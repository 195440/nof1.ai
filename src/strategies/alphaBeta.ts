...
export function getAlphaBetaStrategy(maxLeverage: number): StrategyParams {
  ...
  return {
    ...
    // ==================== 止损配置（ROI%，已包含杠杆）====================
    // 用户反馈：止损-10~-15%相对止盈3%差距太大
    // 必须大幅收紧！配合止盈3%，止损应该在-4~-6%
    stopLoss: {
      low: -6,    // 低杠杆，快速止损
      mid: -5,    // 中杠杆，更快止损
      high: -4,   // 高杠杆，最快止损
    ...
    // ==================== 分批止盈配置（ROI%，已包含杠杆）====================
    // 用户反馈：盈利接近5%时未触发，然后反转触发止损
    // 必须更激进地锁定利润！第一时间落袋为安！
    partialTakeProfit: {
      stage1: { trigger: 3, closePercent: 60 },   // 3%立即平60%，快速锁定利润
      stage2: { trigger: 8, closePercent: 90 },   // 8%平到累计90%
      stage3: { trigger: 12, closePercent: 100 }, // 12%全平
    },
...
4) **仓位与杠杆必须动态计算**，严禁固定值
5) **盈利要落袋**：让利润奔跑，但用分批止盈+移动止盈把利润锁进账户（代码级保护已启用）
6) **快速止损，让利润奔跑**：止损要比止盈更果断，单笔亏损不超过预期盈利的50%
...
- 自动分批止盈：+${params.partialTakeProfit.stage1.trigger}% 平至累计${params.partialTakeProfit.stage1.closePercent}% → +${params.partialTakeProfit.stage2.trigger}% 平至累计${params.partialTakeProfit.stage2.closePercent}% → +${params.partialTakeProfit.stage3.trigger}% 全平
...
### Range（震荡战术）
- 只做边界均值回归，且更苛刻：RSI7 极端（≤25/≥75）+ 价格接近区间边界 + 动量出现反转迹象
- 震荡门槛更高：手续费会吃掉你的期望值
...
开仓门槛（硬规则）：
- Trend：最高分方向 ≥ 80（原78，提高6分）
- Range：最高分方向 ≥ 85（原85，提高3分）
- Chaos：最高分方向 ≥ 92（原90，提高2分） 且必须解释“为什么不是噪音”
...
建议映射（保守为主）：
- Trend & 分数 88-92：杠杆 3-5x，仓位 10-15%
- Trend & 分数 93-95：杠杆 5-7x，仓位 15-20%
- Trend & 分数 ≥96：杠杆 7-10x，仓位 20-25%
- Range：杠杆 3-4x，仓位 8-12%
- Chaos：杠杆2-3x，仓位5-8%（或观望）
...
2) 分批止盈情况（严格执行，快速锁定利润）：
   - 盈利 ≥ +3% → closePosition 60%（立即锁定大部分利润！）
   - 盈利 ≥ +8% → closePosition 累计90%
   - 盈利 ≥ +12% → closePosition 100%

3) 盈利保护（修改）：
   - 盈利在 +1% ~ +3% 之间 → **继续持有**，等待达到3%第一档
   - 盈利 ≥ +3% → **立即执行分批止盈**，不要犹豫
   - 除非：趋势明确失效 → 立即全部平仓
   - 理由：用户反馈盈利接近5%时未止盈，然后反转亏损，必须更快锁定利润
...
- 连续亏损保护：如连续3笔亏损，该方向评分门槛+5分，重新评估策略

⚠️ **从失败中学习**：
- 如果某币种亏损 → 下次该币种评分门槛提高到85分
- 如果某方向胜率<50% → 该方向评分门槛+5分
- 专注提高开仓质量，用更严格的标准替代禁止交易

...
15) **逆向思维铁律**：买在恐慌（RSI<30），卖在贪婪（RSI>70）；绝不追涨杀跌
...
🚫 **RSI极值禁区（硬性规则）**：
- RSI7 < 25 或 RSI14 < 30：禁止做空，考虑做多机会
- RSI7 > 75 或 RSI14 > 70：禁止做多，考虑做空机会
- 例外：只有在极强趋势（3个周期同向）+ 放量突破时可忽略
...
- **优先逆向交易**：
  * 上涨趋势：等待回调到支撑位（RSI<40）做多
  * 下跌趋势：等待反弹到阻力位（RSI>60）做空
- **避免追涨杀跌**：
  * 禁止在突破后追单，必须等待回踩确认
  * 禁止在急涨急跌后开仓，等待盘整
- 量能确认（提高标准）：
  * 趋势延续：volume/avgVolume ≥ 0.20（避免低量陷阱）
  * 放量突破：volume/avgVolume ≥ 0.50（需要明显放量）
...
3) 动量质量（20）：
   - 正常区域（RSI 30-70）：MACD与RSI支持继续走该方向
   - **RSI极值反向加分**：
     * RSI<30时：做多+10分，做空-10分
     * RSI>70时：做空+10分，做多-10分
...
2) 成交量 ≥ 0.20倍平均成交量（提高门槛，避免低量陷阱）
3) **RSI极值过滤（新增）**：
   - RSI14 < 30：禁止做空，只能做多
   - RSI14 > 70：禁止做多，只能做空
   - RSI7 < 20 或 > 80：观望（极端超买超卖）
4) 该币种上次亏损 → 本次评分门槛提高到 ≥ 75（温和提高）
5) 该币种连续2次亏损 → 评分门槛提高到 ≥ 78（适度提高）
6) 该方向最近3笔胜率<50% → 该方向评分门槛+3分（减少惩罚）
---
...
4. ✅ 成交量≥0.20倍平均？（提高门槛，避免低量陷阱）
5. ✅ RSI不在极值禁区？（RSI<30不做空，RSI>70不做多）
6. ✅ 历史表现检查通过？
7. ✅ 根据历史表现调整：
   - 胜率<40% → 评分门槛+5分
   - 币种上次亏损 → 评分门槛+3分

 ...
 * 
 * 完全复刻 Alpha Arena (nof1.ai) 的设计理念：
 * - 零策略指导，只提供原始市场数据
 * - AI 完全自主决策
...
export function getDeepSeekAlphaArenaStrategy(maxLeverage: number): StrategyParams {
  return {
    ...
    // 这是 Alpha Arena 的核心设计：代码保护是安全网，AI有完全主动权
    allowAiOverrideProtection: true,
  };
}

/**
 * 生成 DeepSeek Alpha Arena 策略特有的提示词
 * 
 ...
 * - 强调双重防护机制
 * 
 * @param params - 策略参数配置（从 getDeepSeekAlphaArenaStrategy 获得）
 * @param context - 运行时上下文（包含执行周期、持仓数量等）
 * @returns DeepSeek Alpha Arena 策略专属的AI提示词
 */

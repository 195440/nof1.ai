...
11. **行情识别与应对策略（核心生存法则）**：
   ...
   (1) 单边行情（趋势行情）- 积极把握，这是赚钱的黄金时期
       * **识别标准**（至少满足3项）：
         ...
         - MACD柱状图连续同向扩大，没有频繁交叉
         - RSI持续在超买区（>70）或超卖区（<30），显示强劲动能
         - 多个时间框架（1m、3m、5m、15m）方向高度一致，无反向信号
         - 价格K线连续同向突破，回调幅度小（<2-3%）
         - 成交量持续放大，显示强劲参与度
       
       * **交易策略**（${params.name === '激进' ? '激进模式必须全力把握' : '积极参与'}）：
         - 入场条件：${params.name === '激进' ? '2个时间框架一致即可入场，不要犹豫！' : '至少3个时间框架一致'}
         - 仓位配置：${params.name === '激进' ? '使用较大仓位（28-32%），充分把握趋势' : '标准仓位'}
         - 杠杆选择：${params.name === '激进' ? '积极使用较高杠杆（22-25倍），抓住机会' : '根据信号强度选择'}
         - 持仓管理：让利润充分奔跑，不要轻易平仓，只在趋势明显减弱时止盈
         - 止损设置：适度放宽止损（给趋势空间），但仍需严格执行
         - 加仓策略：盈利>5%且趋势继续强化时，积极加仓（最多50%原仓位）
         - ${params.name === '激进' ? '关键提醒：单边行情是激进策略的核心盈利来源，必须全力把握！' : ''}
       
       * **单边行情示例**：
         - 做多：价格持续在EMA20上方运行，MACD柱状图连续红色扩大，RSI>70但价格仍在上涨
         - 做空：价格持续在EMA20下方运行，MACD柱状图连续绿色扩大，RSI<30但价格仍在下跌
   ...
   (2) 震荡行情（横盘整理）- 严格防守，避免频繁交易亏损
       * **识别标准**（出现任意2项即判定为震荡）：
         ...
         - 多个时间框架信号不一致或频繁切换（如1m做多、3m做空、5m做多）
       ...
       * **交易策略**（${params.name === '激进' ? '震荡行情是激进策略的死敌，必须严格防守' : '谨慎观望'}）：
         ...
         - 入场条件：${params.name === '激进' ? '至少4个时间框架完全一致才能入场（提高标准！）' : '至少3个时间框架一致'}
       ...
       * **震荡行情示例**：
         - BTC在42000-43000之间反复震荡，MACD频繁交叉，1m做多但5m做空
         - ETH在2200-2250之间横盘，RSI在45-55反复，多个时间框架信号混乱
   ...
   (3) 行情转换识别（关键时刻）：
       * **震荡转单边**（机会信号）：
         ...
         - 多个时间框架同时转向一致方向
         - 成交量突然放大（>平均成交量150%）
         - ${params.name === '激进' ? '这是入场的最佳时机，抓住突破初期！' : '这是重要的入场机会'}
   ...
   (4) ${params.name === '激进' ? '激进策略特别提醒' : '策略总结'}：
       ${params.name === '激进' ? `- 单边行情 = 全力进攻 = 大仓位 + 高杠杆 + 积极加仓 = 赚钱的主要来源
       - 震荡行情 = 严格防守 = 小仓位 + 低杠杆 + 高标准 = 避免亏损的关键
       - 成功的激进策略 = 在对的行情做对的事（单边进攻、震荡防守）
       - 失败的激进策略 = 不分行情乱开仓（震荡频繁交易=送钱）
       - 记住：震荡行情频繁开仓是最近一直亏损的根本原因！` : `- 在单边行情积极把握，让利润充分奔跑
       - 在震荡行情谨慎防守，避免频繁交易
       - 正确识别行情类型，调整交易策略
       - 耐心等待高质量机会，不要强行交易`}

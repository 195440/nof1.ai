...
${reverseTradingEnabled ? '⚠️ 反向交易模式：AI决策方向与实际开仓方向相反' : ''}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
${reverseTradingEnabled ? `【⚠️ 反向交易模式说明】
┌─────────────────────────────────────────┐
│ 重要：系统已启用反向交易模式！            │
│ • AI决策做多 → 实际开仓做空               │
│ • AI决策做空 → 实际开仓做多               │
...
4. 评估新交易机会（如果决定开仓，必须立即执行）：
   ...
   a) 加仓评估（对已有盈利持仓）：
      - ${reverseTradingEnabled ? '【反向交易模式】持仓方向判断：比较AI当前判断与持仓方向（非实际开仓方向）\n      - 方向正确 = AI当前判断与之前决策一致（如：之前AI判断做多→实际做空，现在AI仍判断做多）\n      - 方向错误 = AI当前判断与之前决策相反（如：之前AI判断做多→实际做空，现在AI判断做空）\n      - ' : ''}该币种已有持仓且方向正确
   ...
   b) 新开仓评估（新币种）：
      ...
      - ${reverseTradingEnabled ? '【反向交易模式】开仓方向说明：\n        * AI判断做多 → 实际开仓做空（系统会自动反转）\n        * AI判断做空 → 实际开仓做多（系统会自动反转）\n        * 所以AI应专注于技术分析，方向反转由系统处理\n        * ' : ''}做多和做空机会的识别：
...
${reverseTradingEnabled ? '- 【反向交易模式】AI决策方向与实际开仓方向相反，AI应专注技术分析\n' : ''}- 允许加仓：对盈利>5%的持仓，趋势强化时可加仓≤50%，最多2次
...
${reverseTradingEnabled ? '- **反向交易模式**：AI专注技术分析，方向反转由系统自动处理\n' : ''}- **数据驱动+经验判断**：基于技术指标和多时间框架分析，同时运用您的专业判断和市场洞察力

...
export function getTradingStrategy(): TradingStrategy {
  ...
  if (strategy === "conservative" || strategy === "balanced" || strategy === "aggressive" || strategy === "aggressive-team" || strategy === "ultra-short" || strategy === "swing-trend" || strategy === "medium-long" || strategy === "rebate-farming" || strategy === "ai-autonomous" || strategy === "multi-agent-consensus" || strategy === "deepseek-alpha") {
    return strategy;
  }
...
export function generateTradingPrompt(data: {
  ...
  // 如果是AI自主策略或DeepSeek Alpha Arena策略，使用完全不同的提示词格式
  if (strategy === "ai-autonomous" || strategy === "deepseek-alpha") {
...
function generateInstructions(strategy: TradingStrategy, intervalMinutes: number): string {
  ...
  // 如果是AI自主策略或DeepSeek Alpha Arena策略，返回极简的系统提示词
  if (strategy === "ai-autonomous" || strategy === "deepseek-alpha") {
    const strategyName = strategy === "deepseek-alpha" ? "DeepSeek Alpha Arena" : "AI自主";
    const strategyDesc = strategy === "deepseek-alpha" 
      ? "这是对 Alpha Arena (nof1.ai) 的完整复刻，你的所有行为都会被记录和分析。" 
      : "";
